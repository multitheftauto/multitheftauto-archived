...
{
	//DWORD dwFunc = FUNC_CAudioEngine_StopRadio;
...
void CAudioEngineSA::PlayFrontEndSound(DWORD dwSound)
{
    if  ( *(DWORD *)VAR_AudioEventVolumes != 0 && dwSound <= 101 ) // may prevent a crash
    {
	    ...
	    DWORD dwFunc = FUNC_CAudioEngine_ReportFrontendAudioEvent;
	    FLOAT fUnknown = 1.0f;
	    _asm
	    {
		    push	fUnknown
...
{
	DWORD dwFunc = FUNC_CAudioEngine_SetEffectsMasterVolume;
...
{
	DWORD dwFunc = FUNC_CAudioEngine_SetMusicMasterVolume;
	DWORD dwVolume = bVolume;
	_asm
	{
		mov		ecx, CLASS_CAudioEngine
...
void CAudioEngineSA::PlayBeatTrack ( short iTrack )
{
    if  ( *(DWORD *)VAR_AudioEventVolumes != 0 ) // may prevent a crash
    {
	    ...
	    DWORD dwFunc = FUNC_CAudioEngine_PreloadBeatTrack;
	    ...
	    dwFunc = FUNC_CAudioEngine_PlayPreloadedBeatTrack;
	    _asm
	    {
		    mov		ecx, CLASS_CAudioEngine
		    push	1
...
void CAudioEngineSA::PauseAllSound ( bool bPaused )
{
    if ( bPaused )
    {
        DWORD dwFunc = FUNC_CAudioEngine_PauseAllSounds;
	    _asm	
	    {
		    mov		ecx, CLASS_CAudioEngine
		    call	dwFunc
    ...
    {
        DWORD dwFunc = FUNC_CAudioEngine_ResumeAllSounds;
	    _asm	
	    {
		    mov		ecx, CLASS_CAudioEngine
		    call	dwFunc
