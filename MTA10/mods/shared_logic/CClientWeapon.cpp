...
CClientWeapon::CClientWeapon ( CClientManager * pManager, ElementID ID, eWeaponType type ) : CClientObject ( pManager, ID, CClientPickupManager::GetWeaponModel ( type ) )
{
    m_pManager = pManager;
...
void CClientWeapon::FireInstantHit ( CVector & vecOrigin, CVector & vecTarget )
{
    ...
    if ( GetAttachedTo () ) g_pGame->GetFx ()->TriggerGunshot ( m_pObject, vecOrigin, vecDirection, false );
    ...
    if ( g_pGame->GetWorld ()->ProcessLineOfSight ( &vecOrigin, &vecTarget, &pColPoint, &pColEntity ) )
    {
